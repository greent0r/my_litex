    ...
    def __init__(self, sys_clk_freq=50e6,
        ...
        **kwargs):
        ...
        if with_uart:
            ...
            uart_phy  = UARTPHY(platform.request("uart"), clk_freq=self.sys_clk_freq, baudrate=115200)
            self.uart = UART(uart_phy)
            # self.add_module(name = f"uart_phy", module = uart_phy)
            # self.add_module(name = "uart", module = self.uart)
            # self.irq.add("uart", use_loc_if_exists=True)

        # GPIO -------------------------------------------------------------------------------------
        if with_gpio:
            self.gpioA = GPIOTristate(
                pads         = platform.request("gpioA"),
                with_irq     = self.irq.enabled
            )
            self.irq.add("gpioA", use_loc_if_exists=True)

            self.gpioB = GPIOTristate(
                pads         = platform.request("gpioB"),
                with_irq     = self.irq.enabled
            )
            self.irq.add("gpioB", use_loc_if_exists=True)
        ...
        if with_i2c:
            ...
            self.irq.add("i2c", use_loc_if_exists=True)
        ...
        if with_spi:
            ...
            self.spi_ctl = GPIOOut(self.platform.request("spi_ctl"))
